...
struct ExpenseCategoryButton: View {
    @State var expenseCategoryName: String
        ...
        VStack {
            ...
            ScrollView {
                LazyVGrid(columns: gridLayout, spacing: 0) {
                    // MARK: - 카테고리 버튼
                    ForEach(ExpenseCategoryList.allCases, id: \.self) { expeseCategory in
                        let expenseCategoryName = ExpenseCategoryList.RawValue()
                        ExpenseCategoryButton(expenseCategoryName: expeseCategory.rawValue, expenseCategoryColor: expeseCategory.expenseCategoryListColor)
                            .onTapGesture {
